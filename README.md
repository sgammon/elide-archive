...
If you'd like to use *Elide* for your own app, drop us a line by filing an issue, and we'll help you get there. *Elide* is already in use for production apps, but only inside our own team - so we can support it, but only on an individual/one-off basis, for now.
...
builder-image                  Build a new version of the CI builder image for Elide.
